...
from .models import PurchaseOrderItem, PurchaseOrder, ImportOrder, \
    PaymentHistory, ImportOrderItem
...
class PurchaseOrderForm(forms.ModelForm):
    class Meta:
        model = PurchaseOrder
        fields = ['finish_pay', 'handler', 'date', 'supplier', 'cost_money',
                  'cost_by', 'ps', 'file']
        widgets = {
...
class PurchaseOrderItemForm(forms.ModelForm):
    ...
    weight = forms.DecimalField(label='重量', max_digits=9, decimal_places=2,
                                help_text='单位为：吨', required=False)
    ...
    def save(self, commit=True):
        ...
        cd['quarry'], _ = Quarry.objects.get_or_create(
            name=cd['quarry'].upper())
        cd['category'], _ = Category.objects.get_or_create(
            name=cd['category'].upper())
        product_dict = {'weight': cd['weight'], 'long': cd['long'],
                        'width': cd['width'],
                        'high': cd['high'], 'm3': cd['m3'],
                        'batch': cd['batch'],
                        'ps': cd['ps'], 'quarry': cd['quarry'],
                        'category': cd['category']}
        cd['block_num'], _ = Product.objects.get_or_create(block_num=block_name,
...
def get_choices_list():
    purchase_order = [(i.order, i.order + '[采购]') for i in
                      PurchaseOrder.objects.filter(finish_pay=False)]
    import_order = [(i.order, i.order + '[运输]') for i in
                    ImportOrder.objects.filter(finish_pay=False)]
...
class PaymentForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields['order'] = forms.ChoiceField(label='相关订单号',
                                                 widget=forms.Select,
                                                 required=False,
...
class PurchaseOrderItemBaseInlineFormset(forms.BaseInlineFormSet):
    def __init__(self, *args, **kwargs):
        super(PurchaseOrderItemBaseInlineFormset, self).__init__(*args,
                                                                 **kwargs)
